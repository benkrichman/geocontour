...
def dim(dimension,exit_on_error=True):
    """
    ...
    spacing=numpy.diff(dimension)
    if not numpy.all(spacing==spacing[0]):
        if not numpy.allclose(spacing,spacing[0],rtol=1e-10,atol=1e-10):
            if exit_on_error:
...
def contour(contour,latitudes=None,longitudes=None):
    """
    ...
    if latitudes is not None and longitudes is not None:
        checkdim(longitudes)
        checkdim(latitudes)
        lonspc=abs(numpy.diff(longitudes)[0])
        latspc=abs(numpy.diff(latitudes)[0])
        spacing=numpy.array([latspc,lonspc])
        ...
        if not ((abs(contourdiff)==0)+(abs(contourdiff)==spacing)+(abs(contourdiff)==numpy.linalg.norm(spacing))).all():
            sys.exit('ERROR - Contour does not seem to be connected, ensure contour is continuous (8-connected or 4-connected)')
    else:
        spacing=numpy.array([1,1])
        if not ((abs(contourdiff)==0)+(abs(contourdiff)==spacing)+(abs(contourdiff)==numpy.linalg.norm(spacing))).all():
            sys.exit('ERROR - Contour does not seem to be connected, ensure contour is continuous (8-connected or 4-connected)')
...
def geocontour(geocontour,latitudes,longitudes):
    """
